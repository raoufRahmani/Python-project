from version1 import Dataset, regression, results
...
def test_add_intercept():
    ...
    assert data_1.X.shape[0] == n_before ,f"The number of samples should remain {n_before} after adding intercept."
    assert data_1.X.shape[1] == p_before + 1 ,f"The number of features should be {p_before + 1}. after adding intercept." 
    assert data_1.features_names[0] == "constante","The first column name should be 'constante'."
    assert len(data_1.features_names) == p_before + 1, "The Feature names list length is incorrect."
